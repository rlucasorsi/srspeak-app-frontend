...
const mockModule = {
  ...
  cards: [
    { id: 1, english: "Where is the bathroom?", portuguese: "Onde fica o banheiro?", difficulty: "easy" },
    { id: 2, english: "I would like to order, please.", portuguese: "Eu gostaria de fazer o pedido, por favor.", difficulty: "medium" },
    { id: 3, english: "How much does it cost?", portuguese: "Quanto custa?", difficulty: "easy" },
  ]
};
...
export default function ModuleFormPage() {
  ...
  const [category, setCategory] = useState(isEditMode ? mockModule.category : "");
  const [description, setDescription] = useState(isEditMode ? mockModule.description : "");
  const [cards, setCards] = useState<Card[]>(isEditMode ? mockModule.cards : []);
        ...
        <Card className="mt-6">
          ...
          <CardContent>
            <div className="space-y-4">
              {cards.map((card) => (
                <div key={card.id} className="p-4 border rounded-lg flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">
                      {difficultyOptions.find(opt => opt.value === card.difficulty)?.label || card.difficulty}
                    </span>
                  </div>
                </div>
              ))}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input 
                      ...
                      onChange={(e) => setNewCard({...newCard, portuguese: e.target.value})} 
          ...
          <CardFooter className="flex justify-end border-t pt-4">
            <Button variant="outline" className="mr-2" onClick={() => navigate("/modules")}>
              Cancelar
            </Button>
            <Button onClick={handleSaveModule} disabled={!name || cards.length === 0}>
